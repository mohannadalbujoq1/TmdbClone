...
function Movies({ setIsLoading, setLoadingProgress, isSearchVisible }) {
  ...
  const getMovie = useCallback((page = 1, sortOption = 'popularity.desc', reset = false, query = '') => {
    ...
    axios.get(url)
      .then(response => {
        ...
        setIsSearchActive(!!query.length); // Update based on whether a query is present
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.innerHeight + document.documentElement.scrollTop === document.documentElement.offsetHeight && hasClickedLoadMore && !isSearchActive) {
        handleNextPage();
      }
    };
  ...
  const handleSearch = (query) => {
    ...
    setHasClickedLoadMore(false); // Reset load more behavior on search
  };
    ...
    <div>
      {isSearchVisible && <Search onSearch={handleSearch} />}
        ...
        <>
          ...
          <ButtonContainer isSearchActive={isSearchActive} data-testid="load-more-button" onClick={() => { handleNextPage(); setHasClickedLoadMore(true); }}>
